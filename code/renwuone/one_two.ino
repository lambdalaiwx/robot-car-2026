...
void setup() {
  ...
  // 给舵机 1.5 秒的物理运动时间，让它彻底停稳
  delay(1500); 
  
  // --- ⚠️ 阶段 2：核心黑科技 - 强制释放与重置 Timer1 ---
  ...
  // 底层寄存器劫持：强制恢复 Arduino Timer1 的标准 8-bit PWM 模式
  TCCR1A = _BV(WGM10);
  ...
  delay(450);
  stopCar();
  delay(2000); // 走位结束，休息 2 秒准备进入视觉模式
...
} // <--- 修复点：这里才是整个 setup() 结束的地方！

// ==========================================
// 主循环 Loop (不断监听 OpenMV)
// ==========================================
void loop() {
